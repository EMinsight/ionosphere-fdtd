...
조사 결과 부동소수점 정밀도, FFT zero-padding, DFT 절단 지점 선택, source-plane 반올림은 주원인이 아닌 것으로 판명됐다. 균일 모델과 방위각 연구는 공간 분산이 실제로 존재하며 수렴함을 보여준다. Subdivision 8에서는 평가 대역의 방향 비등방성이 0.0867% 이하로 작다. 따라서 남은 절대 잔차에는 등방성 공간 분산과 유한한 방사 및 지각 모델의 차이도 포함된다.
...
375 Hz 이상에서 평균/최대 spread는 level 6의 0.949%/1.334%에서 level 8의 0.0508%/0.0866%로 줄었다. 따라서 약 400 Hz 이상에서 나타난 급격한 level-5 분기는 해상도 부족 효과이며, 일반적인 격자 세분화만으로 방향 오차는 사실상 제거할 수 있다.

모든 방위각에 공통인 최소, 중앙, 최대 적응형 절단 지점을 적용하면 levels 6, 7, 8의 평균 spread 범위는 각각 0.4678–0.4680%, 0.1154–0.1162%, 0.02422–0.02452%다. 최대 spread 범위도 1.288–1.334%, 0.4617–0.4969%, 0.08621–0.08724%다. 따라서 방향별 DFT window 선택 때문에 수렴하는 것처럼 보이는 현상은 아니다.
