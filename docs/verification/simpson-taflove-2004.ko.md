...
이어 방사와 시간 간격을 함께 세분화하여 5 km 방사 간격의 영향을 수평 분산과 분리했다. 시간 간격만 절반으로 줄였을 때 균일 level-5 평균 위상속도 오차는 `5.05e-8 c`만 변하므로 시간 적분 오차는 원인에서 제외할 수 있다. 방사 간격을 절반으로 줄인 결과는 일관되지 않았다. Subdivision 5에서는 평균 오차가 5.2% 줄었지만 subdivision 6에서는 8.5% 늘었다. 반면 수평 격자를 subdivision 5에서 6으로 세분화하면 평균 오차가 5 km 방사 격자에서 53.3%, 2.5 km 격자에서 46.5% 감소한다. 따라서 비용이 큰 level-8 방사 세분화는 선별 기준을 통과하지 못해 실행하지 않았다. 다음 수치 개선 대상은 더 작은 시간 간격이나 균일 방사 세분화가 아니라 수평 등방 연산자다.
...
세 전 지구 손실은 0.21% 안에서 일치한다. ETOPO5는 B가 10분의 1로 약해졌는데도 회랑 손실비가 가장 대칭적이다. B 도달 시점과 가까운 step 15,360에서 ETOPO5 동쪽 회랑의 표본 field energy는 `5.28e-8 J`로, 서쪽의 `4.12e-8 J`보다 28% 많다. 따라서 약한 B 값은 동쪽 경로의 과도한 누적 손실로 펄스가 사라진 결과가 아니다.

### 수신점 지지 자유도와 해안선 aliasing

적도 동경 43°의 정확한 B 좌표에서 ETOPO5 고도는 −207 m다. 그런데 subdivision-5 barycentric receiver는 가중치의 88.8869%를 동경 42.75°의 인접 dual vertex에 할당하며, 이 vertex의 표면고도는 +30 m다. B 피크에서 이 육지 vertex의 값은 거의 0인 반면 두 해양 지지 vertex는 모두 약 −0.36 μV/m다. 따라서 이 이상은 보간값의 상쇄가 아니다. 지배적인 지지 자유도의 해수면 `Er` 평면 전체에 암석 물질이 할당된 결과다.
...
일관된 효과는 수평 세분화에서 나타난다. Subdivision 5→6은 5 km에서 평균 및 최대 위상오차를 53.3%, 47.3% 줄이고, 2.5 km에서는 46.5%, 43.8% 줄인다. 따라서 균일 방사 세분화를 level-8 프로덕션 실행으로 올리지 않았다. 비용이 약 네 배이고 논문의 5 km 이산화와도 달라지며, 저해상도 개선 기준도 통과하지 못했다. 남은 수치 오차 중 실제로 다룰 수 있는 부분은 수평 측지 연산자의 주파수 의존 등방 분산이다. 격자 topology를 유지하면서 이를 줄이려면 고차 또는 분산 최적화 Hodge/curl 구성이 필요하다. 주파수 의존성이 있으므로 하나의 상수로 파동속도를 재조정하는 방법은 맞지 않는다.
...
| 수평 subdivision 5→6 | 두 방사 격자에서 평균 오차 53.3% / 46.5% 감소 | 수평 등방 분산이 지배적임 |
