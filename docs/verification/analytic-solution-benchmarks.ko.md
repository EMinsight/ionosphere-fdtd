...
| A2 | 무손실 thin shell의 구면 surface harmonic | Geodesic curl/Hodge metric, TM/TE branch, leapfrog 시간 적분 | `lambda_l=l(l+1)/R^2`, `f_l=c sqrt(lambda_l)/(2 pi)`, 정확한 leapfrog 주파수 | 수식과 component test 준비 완료, production 수렴 runner가 다음 작업 |
| A3 | 균일 손실 매질의 plane wave | 유전율, 전도도, 감쇠와 위상 부호 | `gamma=sqrt[j omega mu (sigma+j omega epsilon)]` | 수식과 test 준비 완료, end-to-end 실행에는 plane-wave/periodic 보조 geometry 필요 |
| A4 | 두 동심 PEC 구면 사이 vector spherical harmonic | 완전한 구면 방사 metric, 모든 field component, 방사 PEC 경계, modal frequency 추출 | TE/TM spherical-Bessel determinant root | Root solver 준비 완료, staggered-field initializer와 projection runner가 다음 구현 대상 |
...
3. A2 discrete-time trace는 leapfrog recurrence와 roundoff 이내로 일치해야
   한다. 연속 고유값 오차는 subdivision 5–8에서 단조 감소하고 관측 차수가
   약 2여야 한다.
...
바로 다음 변경에서는 작은 subdivision부터 A4 staggered vector-spherical-
harmonic initializer와 modal projection을 구현한다. 더 저렴한 A2 regression도
함께 실행한다. A3는 periodic 또는 그 밖의 정확한 plane-wave 보조 geometry가
생길 때까지 미룬다. 전역 point-source solution을 사용하면 analytic 가정이
성립하지 않는다.
